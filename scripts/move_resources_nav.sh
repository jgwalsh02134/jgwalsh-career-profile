...
for f in *.html; do
  perl -0777 -i -pe 's/\s*<li[^>]*>\s*<a[^>]*href="(?:\\/resources\\/?|resources(?:\\/index)?\\.html)"[^>]*>.*?Resources.*?<\\/a>\s*<\\/li>\s*//igs' "$f"
  perl -0777 -i -pe 's/\s*<a[^>]*href="(?:\\/resources\\/?|resources(?:\\/index)?\\.html)"[^>]*>\s*Resources\s*<\\/a>\s*//igs' "$f"
  # Clean up any multiple blank lines produced
  perl -0777 -i -pe 's/\n{3,}/\n\n/g' "$f"
